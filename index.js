...
class Action {
    ...
    constructor() {
        ...
        const existingSources = this._executeCommand("nuget list source", { encoding: "utf8" }).stdout;
        if(existingSources.includes(this.nugetSource) === false) {
            let addSourceCmd;
            if (this.nugetSource.startsWith(`https://nuget.pkg.github.com/`)) {
                this.sourceType = "GPR"
                addSourceCmd = `nuget add source ${this.nugetSource}/index.json --name=${(SOURCE_NAME)} --username=${this.githubUser} --password=${this.nugetKey} --store-password-in-clear-text`
            } else {
                this.sourceType = "NuGet"
                addSourceCmd = `nuget add source ${this.nugetSource}/v3/index.json --name=${SOURCE_NAME}`
            }
        ...
        const list1 = this._executeCommand("nuget list source", { encoding: "utf8" }).stdout;
        const enable = this._executeCommand(`nuget enable source ${SOURCE_NAME}`, { encoding: "utf8" }).stdout;
        console.log(list1);
        console.log(enable);
    }
    ...
    _pushPackage(version, name) {
        ...
        const pushCmd = `nuget push *.nupkg -s ${(SOURCE_NAME)} ${this.nugetSource !== "GPR"? `-k ${this.nugetKey}`: ""} --skip-duplicate ${!this.includeSymbols ? "-n 1" : ""}`
